...
const DeleteConfirmModal = ({ isOpen, onClose, blogId, blogTitle }) => {
  ...
  const handleDelete = async () => {
    ...
    if (result && result.success) {
      ...
      alert(`Purging process aborted: ${result.error || "Mongoose casting exceptions."}`);
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl border border-slate-200 w-full max-w-sm overflow-hidden p-6 relative transform transition-all shadow-xl text-sm">
        ...
        <button onClick={onClose} className="absolute right-4 top-4 p-1 rounded-md text-slate-400 hover:bg-slate-100 transition-colors">
          <X size={16} />
        </button>
        ...
        <div className="flex items-start gap-3.5">
          ...
          <div>
            ...
            <p className="text-slate-500 leading-relaxed text-xs">
              Confirm permanent destruction of <span className="font-semibold text-slate-800">"{blogTitle}"</span> inside MongoDB indexes. This operation cannot be reversed.
            </p>
          </div>
        </div>
